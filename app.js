...
const state = {
  ...
  selectedId: null,
  selectedLinkId: null,   // id of the CHILD node whose parent link is selected
  favorite: false,
  dirty: false
};
...
/* ============ HISTORY ============ */
function snapshot() {
  history.past.push(JSON.stringify({ nodes: state.nodes, bgColor: state.bgColor }));
...
function undo() {
  ...
  history.future.push(JSON.stringify({ nodes: state.nodes, bgColor: state.bgColor }));
...
async function deleteProjectAndRefresh(name) {
  try {
    ...
    if (state.activeProject === name) {
      ...
      state.nodes = []; state.selectedId = null;
...
function renderNodes() {
  ...
  for (const n of state.nodes) {
    ...
    const $handle = $('<button class="link-handle" title="Drag onto another node to connect / re-connect this node to it"><i class="fa-solid fa-link"></i></button>');
...
function rebuildLinkSprings() {
  const valid = new Set();
  for (const n of state.nodes) if (n.parentId && getNode(n.parentId)) valid.add(n.parentId + '>' + n.id);
  for (const k of [...linkSprings.keys()]) if (!valid.has(k)) linkSprings.delete(k);
  for (const n of state.nodes) {
    if (n.parentId && getNode(n.parentId)) {
      const k = n.parentId + '>' + n.id;
      if (!linkSprings.has(k)) {
        const p = getNode(n.parentId);
        const dx = (n.x - p.x) * 0.5;
        linkSprings.set(k, { cp1x: p.x+dx, cp1y: p.y, cp2x: n.x-dx, cp2y: n.y,
...
function tickSprings() {
  ...
  for (const n of state.nodes) {
    ...
    const p = getNode(n.parentId); if (!p) continue;
    const s = linkSprings.get(n.parentId + '>' + n.id); if (!s) continue;
...
function renderLinkPaths() {
  ...
  for (const n of state.nodes) {
    if (n.parentId && getNode(n.parentId)) {
      const p = getNode(n.parentId);
      const s = linkSprings.get(n.parentId + '>' + n.id);
      if (s) data.push({ id: n.id, key: n.parentId + '>' + n.id, p, c: n, s });
    }
  }
  ...
  gEnter.merge(groups)
    ...
    .each(function (d) {
      const rev = !!d.c.arrowReversed;   // arrow points child -> parent instead
...
function disconnectLink(childId) {
...
function reverseLinkArrow(childId) {
  const n = getNode(childId);
  if (!n || !n.parentId) return;
  snapshot();
  n.arrowReversed = !n.arrowReversed;
  state.selectedLinkId = childId;  // keep the link selected for repeated toggling
  render();
...
function startRelinkDrag(childId, e) {
  relinkDrag = { childId, hoverId: null };
  $('body').addClass('relinking');
  updateTempLink(e.clientX, e.clientY);
}
function updateTempLink(cx, cy) {
  ...
  if (relinkDrag.hoverId !== overId) {
    ...
    if (overId && overId !== relinkDrag.childId) {
      const invalid = isDescendantOf(overId, relinkDrag.childId);
...
$(document).on('mousemove', e => { if (relinkDrag) updateTempLink(e.clientX, e.clientY); });
$(document).on('mouseup', () => {
  if (!relinkDrag) return;
  const { childId, hoverId } = relinkDrag;
  cancelRelinkDrag();
  if (hoverId && hoverId !== childId) reparentNode(childId, hoverId);
  else if (!hoverId) toast('Drop onto a node to connect');
});

/* click-to-pick parent mode (from the context menu) */
let reparentPickId = null;
function beginReparentPick(id) {
  reparentPickId = id;
  $('body').addClass('picking-parent');
  toast('Click the node you want as the new parent (Esc to cancel)', 2600);
}
...
function bindNodeEvents() {
  $('.node').each(function () {
    ...
    $n.on('mousedown', e => {
      ...
      if (reparentPickId) {
        e.stopPropagation();
        const childId = reparentPickId;
        endReparentPick();
        reparentNode(childId, id);
        return;
      }
...
$('#btn-clear').on('click', () => {
  ...
  snapshot();
  state.nodes = []; state.selectedId = null; state.selectedLinkId = null;
  render();
});
